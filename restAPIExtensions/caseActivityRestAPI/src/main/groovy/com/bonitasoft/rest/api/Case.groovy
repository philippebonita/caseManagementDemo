...
class Case implements RestApiController {
	...
	RestApiResponse doHandle(HttpServletRequest request, RestApiResponseBuilder responseBuilder, RestAPIContext context) {
		...
		def customerDAO = context.apiClient.getDAO(CustomerDAO.class)
		def result = processAPI.searchProcessInstances(new SearchOptionsBuilder(0, 9999).with {
			done()
		}).getResult()
		.collect{
			def ref = processAPI.getProcessInstanceExecutionContext(it.id)["customer_ref"]
			def Customer customer = customerDAO.findByPersistenceId(ref.storageId)
			[id:"$it.id -- $customer.firstName $customer.lastName",name:processName(it.processDefinitionId,processAPI),state:asLabel(it.state.toUpperCase(),"info"),viewAction:viewActionLink(it.id,processAPI,contextPath)]
		}
